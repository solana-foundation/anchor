...
//! [`saturn_bitcoin_transactions::TransactionBuilder`].  *It never mutates the
...
//! Error semantics
//! ---------------
//! * **Arithmetic overflow / underflow** ⇒ [`saturn_safe_math::MathError`]
//! * **Rune-specific validation errors** ⇒ [`crate::StateShardError`]
//!
//! All algorithms are `no_std`-compatible and rely on the fixed-size
//! collections from `saturn_collections`, keeping worst-case memory usage
...
mod tests_loader {
    ...
    mod edge_cases {
        use super::*;
        use crate::tests::common::add_btc_utxos_bulk;
        use crate::tests::common::random_utxo_meta;
        use crate::split::{
        ...
        fn max_capacity_stress() {
            ...
            let mut shards: Vec<MockShardZc> = (0..MAX_SHARDS_PER_PROGRAM)
                .map(|i| {
