        ...
        T: IntoShardIndices<MAX_SELECTED_SHARDS>,
    {
        ...
        for &idx in indexes.as_slice() {
            if idx >= self.loaders.len() {
                return Err(StateShardError::OutOfBounds);
            }
            ...
            if self.selected.iter().any(|&existing| existing == idx) {
                return Err(StateShardError::DuplicateShardSelection);
            }
    ...
    S: ZeroCopy + Owner,
{
    ...
    ) -> Result<Vec<u128>, saturn_safe_math::MathError>
    where
        RS: saturn_collections::generic::fixed_set::FixedCapacitySet<
                Item = arch_program::rune::RuneAmount,
            > + Default,
        U: saturn_bitcoin_transactions::utxo_info::UtxoInfoTrait<RS>,
        S: crate::StateShard<U, RS> + ZeroCopy + Owner,
    {
    ...
    >(
        ...
        removed_from_shards: u64,
        fee_rate: &saturn_bitcoin_transactions::fee_rate::FeeRate,
    ) -> Result<u64, saturn_safe_math::MathError>
    where
        RS: saturn_collections::generic::fixed_set::FixedCapacitySet<
                Item = arch_program::rune::RuneAmount,
            > + Default,
    ...
    ) -> Result<RS, crate::StateShardError>
    where
        RS: saturn_collections::generic::fixed_set::FixedCapacitySet<
                Item = arch_program::rune::RuneAmount,
            > + Default,
        ...
        S: crate::StateShard<U, RS> + ZeroCopy + Owner,
    {
        crate::split::compute_unsettled_rune_in_shards::<RS, U, S, MAX_SELECTED_SHARDS>(
            self,
            removed_from_shards,
        )
    }
    ...
    >(
        ...
        removed_from_shards: RS,
        program_script_pubkey: bitcoin::ScriptBuf,
    ) -> Result<Vec<RS>, crate::StateShardError>
    where
        RS: saturn_collections::generic::fixed_set::FixedCapacitySet<
                Item = arch_program::rune::RuneAmount,
            > + Default,
