...
You can use either the long option (`--url`) or the short option (`-u<cluster_letter>`) for setting your desired cluster.
...
<Callout variant="important" title="Rate limits">
  ...
  ```
  If this occurs, you can get devnet SOL using the [Solana Web Faucet](https://faucet.solana.com).
</Callout>
...
This command creates a new directory with `[2m<project-name>[0m{:ansi}` and initializes a new Anchor project with a modular Rust program structure. By default, tests are written in Rust using the [LiteSVM](https://github.com/LiteSVM/litesvm) crate. Use `--test-template` to choose a different template (`mollusk`, `mocha`, `jest`, etc):
