...
To define an account type that uses zero-copy, annotate the struct with [`#[account(zero_copy)]`](https://github.com/solana-foundation/anchor/blob/62865c636aecc6974fc9cfebfc6cf08ca4f0bb72/lang/attribute/account/src/lib.rs#L419).
...
To deserialize a zero-copy account, use [`AccountLoader<'info, T>`](https://github.com/solana-foundation/anchor/blob/62865c636aecc6974fc9cfebfc6cf08ca4f0bb72/lang/src/accounts/account_loader.rs#L97-L101), where `T` is the zero-copy account type defined with the `#[account(zero_copy)]` attribute.
...
<Callout variant="note">
  The `init` constraint is limited to allocating a maximum of 10240 bytes due to CPI limitations.
  Under the hood, the `init` constraint makes a CPI call to the SystemProgram to create the account.
</Callout>

When initializing a zero-copy account for the first time, use [`load_init`](https://github.com/solana-foundation/anchor/blob/62865c636aecc6974fc9cfebfc6cf08ca4f0bb72/lang/src/accounts/account_loader.rs#L201-L223) to get a mutable reference to the account data. The `load_init` method also sets the account discriminator.
...
For accounts that require more than 10240 bytes, use the [`zero`](https://github.com/solana-foundation/anchor/blob/62865c636aecc6974fc9cfebfc6cf08ca4f0bb72/lang/syn/src/codegen/accounts/constraints.rs#L209-L273) constraint instead of `init`. The `zero` constraint verifies the account has not been initialized by checking that its discriminator has not been set.
...
Use [`load_mut`](https://github.com/solana-foundation/anchor/blob/62865c636aecc6974fc9cfebfc6cf08ca4f0bb72/lang/src/accounts/account_loader.rs#L174-L197) when you need mutable access to update an existing zero-copy account:
...
Use [`load`](https://github.com/solana-foundation/anchor/blob/62865c636aecc6974fc9cfebfc6cf08ca4f0bb72/lang/src/accounts/account_loader.rs#L156-L171) to only read the account data.
