...
Anchor keeps a [CHANGELOG](https://github.com/solana-foundation/anchor/blob/master/CHANGELOG.md) but it's not easy to make sense what has changed, what effect does the change have and how to migrate. This is where release notes come in, an easy to digest and actionable view for each release.
...
- Fields are optional.
- `anchor_version` requires [`avm`](https://github.com/solana-foundation/anchor/tree/master/avm) to be installed.
...
- Even though most of it works great, some parts are still rough around the edges and you may encounter parts that are not fully ironed out. Please [create an issue](https://github.com/solana-foundation/anchor/issues) if you run into a problem.
...
**Note:** This example only works with the default IDL generation method(parsing) for now because type aliases for default Rust types don't work properly with `idl-build`([#2640](https://github.com/solana-foundation/anchor/issues/2640)).
...
`0.29.0` performance is noticeably improved in all areas, the biggest one being [binary size](https://github.com/solana-foundation/anchor/blob/master/bench/BINARY_SIZE.md#0290) which is reduced ~36% compared to `0.28.0`!

Similar benchmarks can be found for [compute units](https://github.com/solana-foundation/anchor/blob/master/bench/COMPUTE_UNITS.md#0290) and [stack memory](https://github.com/solana-foundation/anchor/blob/master/bench/STACK_MEMORY.md#0290).
