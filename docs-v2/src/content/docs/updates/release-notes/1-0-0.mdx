...
`1.0.0` is the first stable major release of Anchor. It ships a large number of breaking changes designed to clean up long-standing technical debt, modernize the toolchain, and establish a stable foundation going forward. We cover the most important changes below, but be sure to check out the full list of changes in the [CHANGELOG](https://github.com/solana-foundation/anchor/blob/v1.0.0/CHANGELOG.md#100---2026-04-02).
...
4. Update TS package(s). Note that the package has been renamed — see [TypeScript package rename](#typescript-package-rename) below.
...
[Surfpool](https://github.com/txtx/surfpool) replaces the local validator as the default backend for `anchor test` and `anchor localnet`. If you want to keep using the `solana-test-validator`, pass `--validator legacy`.
...
The on-chain IDL management instructions that Anchor has used since the beginning have been removed and replaced with the new [Program Metadata Program (PMP)](https://github.com/solana-foundation/anchor/pull/3798). IDL uploads through `anchor deploy` and `anchor idl` commands work as before; only the underlying mechanism has changed.
...
`anchor init` now generates a [LiteSVM](https://github.com/LiteSVM/litesvm) test template by default, replacing the previous TypeScript based default. You can still pick a different template explicitly:
