...
With the addition of deploying the IDL on every program deploy by default, a race condition was introduced that was not found in initial testing. We have added a wait until the program is completely available before continuing the deployment of the IDL.

## Lang

### Fix warnings and prelude Solana-program inclusion

There was still a warning leftover for deprecation of realloc in 0.32.0. This has been remediated in this release.
