...
LiteSVM and Mollusk tests usually set this because the test VM runs inside the Rust process. If the flag is absent and the provider cluster is localnet, `$ [34manchor[0m test{:ansi}` can start a local validator before running the test script.
