...
<Callout title="Tip">
  This is different from the `solana program deploy` command, because every time it's run it will
  generate a _new_ program address.
</Callout>
...
If the configured network is a localnet, then automatically starts the local network and runs the test. By default, [Surfpool](https://github.com/txtx/surfpool) is used as the local network backend. To use `solana-test-validator` instead, pass `--validator legacy`.
