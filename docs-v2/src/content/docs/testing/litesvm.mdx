...
Testing real programs goes beyond simple token transfers. Add a compiled program to a test with [`.add_program_from_file`](https://docs.rs/litesvm/latest/litesvm/struct.LiteSVM.html#method.add_program_from_file).

<Callout variant="tip" title="Pulling a program from a cluster">
  To pull a Solana program from mainnet or devnet, use the `$ [34msolana[0m program dump{:ansi}` command from the Solana CLI.
</Callout>
...
See also: [`warp_to_slot`](https://docs.rs/litesvm/latest/litesvm/struct.LiteSVM.html#method.warp_to_slot), which jumps to a future slot.
...
To copy accounts from mainnet or devnet, use the `$ [34msolana[0m account{:ansi}` command in the Solana CLI to save account data to a file.
