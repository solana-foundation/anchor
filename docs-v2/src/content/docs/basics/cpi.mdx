...
The `lib.rs` file below defines a single `sol_transfer(){:rs}` instruction. When invoked, it issues a CPI to the System Program's `transfer(){:rs}` instruction:

<Code code={lib} lang="rust" title="lib.rs" meta="{10,22}" />

The `cpi.test.ts` file invokes `sol_transfer(){:rs}` and logs a link to the transaction details on SolanaFM:
...
The `lib.rs` file declares a single `sol_transfer(){:rs}` instruction. The sender is a `pda_account{:rs}` derived from the seeds `b"pda"{:rs}` and the recipient's address:

<Code code={libPda} lang="rust" title="lib.rs" meta="{10,17,26}" />

The `cpi.test.ts` file derives the PDA on the client, funds it, then invokes `sol_transfer(){:rs}` to move SOL back to the wallet via a CPI signed by the program:
