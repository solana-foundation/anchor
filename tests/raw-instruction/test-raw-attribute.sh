...
cd "$FAIL_PROGRAM"
if anchor build 2>&1 | grep -q "Functions with &\[u8\] or &mut \[u8\] arguments must be marked with #\[raw\] attribute"; then
...
if anchor build --program-name raw-instruction 2>&1 | tail -3 | grep -q "Finished"; then
