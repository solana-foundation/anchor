...
pub trait BtcTxBuilderAny {}

impl<T> BtcTxBuilderAny for T {}
...
pub struct Context<'a, 'b, 'c, 'info, T: Bumps> {
    ...
    /// Optional Bitcoin transaction builder made available when the user
    /// program is annotated with `btc_tx(...)`.
    pub btc_tx_builder: Option<&'a mut dyn BtcTxBuilderAny>,
}
    ...
    T: fmt::Debug + Bumps,
{
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("Context")
            ...
            .field("btc_tx_builder", &self.btc_tx_builder.is_some())
            .finish()
    }
}
    ...
    T: Bumps + Accounts<'info, T::Bumps>,
{
    ...
    /// Construct a context that carries a Bitcoin transaction builder.
    pub fn with_btc_builder(
        program_id: &'a Pubkey,
        accounts: &'b mut T,
        remaining_accounts: &'c [AccountInfo<'info>],
        bumps: T::Bumps,
        btc_tx_builder: &'a mut dyn BtcTxBuilderAny,
    ) -> Self {
        Self {
            program_id,
            accounts,
            remaining_accounts,
            bumps,
            btc_tx_builder: Some(btc_tx_builder),
        }
    }
}
