...
/// Core trait for Anchor programs, implemented by the `#[program]` macro.
pub trait AnchorProgram {
    /// The program ID, set via `declare_id!` or an inline `const ID: Pubkey` in the
    /// `#[program]` module.
    const ID: Pubkey;
    ...
    /// Validates `program_id` against [`Self::ID`] and dispatches the instruction.
    fn entrypoint<'info>(
        program_id: &Pubkey,
    ...
    /// Dispatches the instruction. Generated by the `#[program]` macro.
    fn dispatch<'info>(
        program_id: &Pubkey,
        accounts: &'info [AccountInfo<'info>],
        data: &[u8],
