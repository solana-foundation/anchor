...
pub mod prelude {
    pub use super::{
        ...
        AccountsClose, AccountsExit, AnchorDeserialize, AnchorProgram, AnchorSerialize,
        Discriminator, DuplicateMutableAccountKeys, Id, InitSpace, Key, Lamports, Owner, Owners,
...
/// The main trait for Anchor programs, automatically implemented by the `#[program]` macro.
///
/// Provides the program ID as an associated constant and the instruction dispatch logic,
/// replacing the hard dependency on a global `declare_id!` constant.
pub trait AnchorProgram {
    /// The expected program ID. Sourced from `declare_id!` (backwards-compatible) or
    /// an inline `const ID: Pubkey` inside the `#[program]` module.
    const ID: Pubkey;

    /// Converts an Anchor error into a [`ProgramError`], logging it first.
    /// Override to customise error handling for this program.
    fn handle_error(err: error::Error) -> ProgramError {
        err.log();
        err.into()
    }

    /// The Solana program entrypoint. Validates the runtime `program_id` against
    /// [`Self::ID`] then calls [`Self::dispatch`].
    fn entrypoint<'info>(
        program_id: &Pubkey,
    ...
    /// Dispatches the instruction to the correct handler.
    /// Generated by the `#[program]` macro.
    fn dispatch<'info>(
        program_id: &Pubkey,
        accounts: &'info [AccountInfo<'info>],
        data: &[u8],
...
/// Blanket [`Id`] impl for any type that implements [`AnchorProgram`].
/// Programs no longer need a separate `declare_id!`-generated `Id` impl.
impl<T: AnchorProgram> Id for T {
    fn id() -> Pubkey {
        T::ID
    }
}
