...
/// [`satellite_bitcoin::utxo_info::UtxoInfo`] inputs.
///
...
pub fn derive_utxo_parser(item: TokenStream) -> TokenStream {
    ...
    let ir = match anchor_syn::parser::utxo::parse::derive_input_to_ir(&input) {
        Ok(ir) => ir,
        Err(e) => return e.to_compile_error().into(),
    };

    // 2) Run semantic validation
    if let Err(e) = anchor_syn::parser::utxo::validate::check(&ir) {
        return e.to_compile_error().into();
    }

    // 3) Generate final implementation
    let expanded = anchor_syn::codegen::utxo::expand(&ir);
