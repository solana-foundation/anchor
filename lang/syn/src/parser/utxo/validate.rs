...
use super::ir::{DeriveInputIr, FieldKind};
...
pub fn check(ir: &DeriveInputIr) -> syn::Result<()> {
    ...
    // ---------------------------------------------------------------------
    for field in &ir.fields {
        if matches!(field.attr.runes, Some(super::ir::RunesPresence::None))
        ...
        if let (Some(super::ir::RunesPresence::Some), Some(expr)) =
...
mod tests {
    ...
    fn ir_from(src: &str) -> ir::DeriveInputIr {
