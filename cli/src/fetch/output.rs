...
mod tests {
    ...
    fn output_names_suffix_same_slot_conflicts() {
        ...
        assert_eq!(
            names
                .get(&(42, "a".into(), IdlHistorySource::Legacy, payload_digest(b"{\"a\":1}")))
                .unwrap(),
            "idl_42_legacy.json"
        );
        assert_eq!(
            names
                .get(&(42, "b".into(), IdlHistorySource::Pmp, payload_digest(b"{\"a\":2}")))
                .unwrap(),
            "idl_42_pmp.json"
        );
    }
    ...
    fn output_names_suffix_duplicate_same_source_conflicts() {
        ...
        assert_eq!(
            names
                .get(&(42, "a".into(), IdlHistorySource::Legacy, payload_digest(b"{\"a\":2}")))
                .unwrap(),
