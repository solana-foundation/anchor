...
Note that in V2, `pinocchio_runtime` is exported from `anchor_lang` (replacing `solana_program`),
so you can remove `solana-program` dependency from your `Cargo.toml` if `pubkey!` was the
