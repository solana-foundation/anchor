...
## Unchecked CPI helpers

For Anchor-generated CPI into another program (`declare_program!` or a callee
with the `cpi` feature), each instruction exposes `foo` and `foo_unchecked`,
where `_unchecked` uses `invoke_signed_unchecked` instead of `invoke_signed`.

Use `_unchecked` only when your caller already supplies correct account metas and
infos for the callee.
