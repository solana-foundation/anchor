...
pub mod cpi {
    ...
    pub fn sol_transfer(ctx: Context<SolTransfer>, amount: u64) -> Result<()> {
        let from_pubkey = ctx.accounts.sender.to_account_view();
        let to_pubkey = ctx.accounts.recipient.to_account_view();
        let program_id = ctx.accounts.system_program.to_account_view();
...
```rust
use anchor_lang::pinocchio_runtime::{program::invoke, system_instruction};
```
...
pub mod cpi {
    ...
    // [!code highlight]
    pub fn sol_transfer(ctx: Context<SolTransfer>, amount: u64) -> Result<()> {
        let from_pubkey = ctx.accounts.pda_account.to_account_view();
        let to_pubkey = ctx.accounts.recipient.to_account_view();
        let program_id = ctx.accounts.system_program.to_account_view();
...
```rust
use anchor_lang::pinocchio_runtime::{program::invoke_signed, system_instruction};
```
