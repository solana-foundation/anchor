...
Description: AccountView can be used as a type but Unchecked Account should be
used instead  
Examples: [Github](https://github.com/solana-developers/anchor-examples/tree/main/account-types/AccountView)
|
[Solpg](https://beta.solpg.io/https://github.com/solana-developers/anchor-examples/tree/main/account-types/AccountView)

```rust title="snippet"
#[derive(Accounts)]
pub struct InstructionAccounts<'info> {
    /// CHECK: AccountView is an unchecked account
    // [!code word:AccountView]
    // [!code highlight]
    pub unchecked_account: AccountView,
}
```
...
Description: Explicit wrapper for AccountView types to emphasize that no checks
are performed  
